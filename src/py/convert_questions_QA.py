...
CORENLP_CACHES = {
    ...
    'QA': 'data/QA/qa_corenlp_cache.json',
    
}
NEARBY_GLOVE_FILE = 'out/nearby_QA_n100_glove_6B_100d.json'
...
def get_qas(dataset):
  ...
  for qa in dataset:
      question = qa['question'].strip()
      answers = qa['answers']
      qas.append((question, answers, qa['qid'], qa['correct_index'],
                   qa['imdb_key'], qa['video_clips'], qa['plot_alignment']))
  return qas
...
def run_corenlp(dataset, qas):
  ...
  with corenlp.CoreNLPServer(port=CORENLP_PORT, logfile=CORENLP_LOG) as server:
    ...
    #for question, answers, context in qas:
    #qas.append((question, answers, qa['imdb_key'], qa['video_clips']))
    for question, answers, qid, correct_index, imdb_key, video_clips, alignment in qas:
...
def alter_questions(qas, alteration_strategy=None):
  ...
  unmatched_qas = []
  num_matched = 0
  for question, answers, qid, correct_index, imdb_key, video_clips, plot_alignment in qas:
  #for question, answers, context in qas:
    parse = corenlp_cache[question]
    tokens = parse['tokens']
    const_parse = read_const_parse(parse['parse'])
    answer = answers[correct_index]
    if not OPTS.quiet:
      print question.encode('utf-8')
...
def dump_data(dataset, prefix, use_answer_placeholder=False, alteration_strategy=None):
  ...
  postag_dict = load_postag_dict()
  out_data = []
  out_obj = {'new': out_data}
  mturk_data = []
  print ("name of dataset is: {} ".format(dataset))
  #for qa in dataset:
  #    question = qa['question'].strip()
  #    answers = qa['answers']
  #    qas.append((question, answers, qa['imdb_key'], qa['video_clips']))
  ...
  for qa in dataset:
    ...
    if use_answer_placeholder:
        ...
        answer = 'ANSWER'
        determiner = ''
        #p_parse = corenlp_cache[qa['question']]
    ...
    original_qa = {
        ...
        'plot_alignment': qa['plot_alignment'],
        'video_clips': qa['video_clips']
    }
    out_qas.append(original_qa)
    if 'train' not in qa['qid']:
        continue
    else:
  ...
  if OPTS.prepend:
    prefix = '%s-pre' % prefix
  with open(os.path.join('out', prefix + '-synonym.json'), 'w') as f:
    json.dump(out_obj, f)
  with open(os.path.join('out', prefix + '-indented-synonym.json'), 'w') as f:
    json.dump(out_obj, f, indent=2)
  with open(os.path.join('out', prefix + '-mturk-synonym.tsv'), 'w') as f:
